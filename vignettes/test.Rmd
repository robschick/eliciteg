...
## Worked Example
Here we'll use the same structure as above with the same number of Dirichlets (3), the same data, and the same $m$ categories. The real trial will include an expanded number of experts, but we'll keep it compact for the sake of the example. Here are the data along with each of the three expert's Dirichlet's. The experts have low, medium, and high confidence, respectively. We create these $n$ by sampling from iteratively larger ranges with `runif():`

```{r}
dir1 <- floor(runif(9, 1, 8))
dir2 <- floor(runif(9, 1, 22))
dir3 <- floor(runif(9, 1, 25))
...
colnames(outdf) <- c('mida2mida', 'mida2seus', 'mida2bof', 'mida2gom', 'mida2gsc', 'mida2jl', 'mida2ne', 'mida2nrth', 'mida2rb')
for(i in 1:nsamp){
  idx <- which(rmultinom(1:3, 1, K) == 1)
  prior <- allDirs[allDirs$expert == idx, 'priors']
