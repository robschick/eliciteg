...
What this shows is that overwhelmingly, expert 2's prior was chosen, which makes sense looking at $k_1$, which was ```r kmat[, 1]```. Expert 2's belief was uniform, i.e. she thought it equally likely that whales would transition to any of the 5 regions. 
...
Recall that the data for this region was: ```r maleExT[, 1]```. So we're essentially just getting the data back since we're adding the same prior for each region.
