...
That output, then, is a matrix where each column is the probability vector (that sums to 1), which we will use to select the expert within the Gibbs loop. For this transition that we're exploring here, Expert 2 is much more likely to be sampled during each step, i.e. their weight is much higher. You can also in the MIDA to all region transition, where the priors are almost identical, we'll be sampling from these two experts almost equally.
...
We'll remake the two supporting matrices, though not show the code, as it's identical to above (save for the dimensions).

```{r realCandK, echo=FALSE}
data(moveData)
