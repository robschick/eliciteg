...
# expert <- dfList[[1]]$expert
# 
# for(j in seq_along(1:nexp)){
#   movePrior <- allqs[[1]][[j]]
#   dfList[[1]][which(dfList[[1]]$expert == expert[j]),]
#   n <- dfList[[1]]$inputConfRaw[which(dfList[[1]]$expert == expert[j])]
#   m <- dfList[[1]]$moveNortherntoMIDA[which(dfList[[1]]$expert == expert[j])] / 100 # part 1 - Northern to MIDA
#   m2 <- dfList[[1]]$moveNortherntoNorthern[which(dfList[[1]]$expert == expert[j])] / 100 # Part 2- Northern to Northern
#   expBeta1a <- betaA(n, m, nreg = nNorthreg)
#   expBeta1a2 <- betaA(n, m2, nreg = nNorthreg)
#   movePrior[rownames(movePrior) == 'MIDA', colnames(movePrior) %in% regNorth] <- expBeta1a
#   diag(movePrior)[which(rownames(movePrior) %in% regNorth)] <- expBeta1a2
#   allqs[[1]][[j]] <- movePrior
# }

# # Question 10 - requires splitting MIDA to Northern transitions `moveMIDAtoNorthern`
# expert <- dfList[[2]]$expert
# 
# for(j in seq_along(1:nexp)){
#   movePrior <- allqs[[2]][[j]]
#   n <- dfList[[2]]$inputConfRaw[which(dfList[[2]]$expert == expert[j])]
#   m <- dfList[[2]]$moveMIDAtoMIDA[which(dfList[[2]]$expert == expert[j])] / 100
#   expBeta1a <- betaA(n, m, nreg = 1)
#   movePrior[rownames(movePrior) == 'MIDA', rownames(movePrior) == 'MIDA'] <- expBeta1a
# 
#   m2 <- dfList[[2]]$moveMIDAtoSEUS[which(dfList[[2]]$expert == expert[j])] / 100
#   expBeta1a2 <-betaA(n, m2, nreg = 1)
#   movePrior[rownames(movePrior) == 'SEUS', colnames(movePrior) == 'MIDA'] <- expBeta1a2
# 
#   m3 <- dfList[[2]]$moveMIDAtoNorthern[which(dfList[[2]]$expert == expert[j])] / 100
...
expert <- dfList[[3]]$expert

for(j in seq_along(1:nexp)){
  movePrior <- allqs[[3]][[j]]
  dfList[[3]][which(dfList[[3]]$expert == expert[j]),]
  n <- dfList[[3]]$inputConfRaw[which(dfList[[3]]$expert == expert[j])]
  m <- dfList[[3]]$moveSEUStoMIDA[which(dfList[[3]]$expert == expert[j])] / 100 # part 1 - SEUS to MIDA
  m2 <- dfList[[3]]$moveSEUStoSEUS[which(dfList[[3]]$expert == expert[j])] / 100 # Part 2- SEUS to SEUS
  expBeta1a <- betaA(n, m, nreg)
  expBeta1a2 <- betaA(n, m2, nreg)
...
### Testing the Algorithm with Data = 0
The goal here is to simply substitute 0 values in for all the data in `maleDat`. If the sampling algorithm is working correctly, then the posterior should simply be the same as the prior (`malePrior`).
